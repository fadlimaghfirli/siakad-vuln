<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 sticky top-24">
            <h3 class="text-xl font-bold text-slate-800 mb-2">Buat Pengumuman</h3>
            <p class="text-sm text-slate-500 mb-6">Tambahkan informasi terbaru ke papan pengumuman kampus.</p>

            <form action="/pengumuman/simpan" method="POST" class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="penulis" value="Mahasiswa / Guest" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 bg-slate-50 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20" required>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Isi Pengumuman</label>
                    <textarea name="isi" rows="5" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20" placeholder="Tuliskan isi pengumuman di sini..." required></textarea>
                </div>
            ...
            <div class="mt-6 p-4 bg-orange-50 rounded-xl border border-orange-100">
                <p class="text-xs text-orange-800 font-medium">💡 <b>Lab Tip:</b> Coba masukkan script XSS ke dalam kolom "Isi Pengumuman" untuk melakukan Stored XSS.</p>
            </div>
        </div>
    </div>
        ...
        <?php else: ?>
            <?php foreach ($pengumuman as $p): ?>
                <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                    ...
                    <h4 class="text-xl font-bold text-slate-900 mb-2"><?= $p['judul'] ?></h4>

                    <div class="flex items-center gap-2 text-xs text-slate-500 mb-4 pb-4 border-b border-slate-100">
                        <span class="bg-brand-50 text-brand-700 px-2.5 py-1 rounded-md font-semibold"><?= $p['penulis'] ?></span>
                        <span>•</span>
                        <span>Diposting pada: <?= date('d M Y', strtotime($p['tanggal'])) ?></span>
                    </div>
